...
def get_source_face(frame: Frame) -> Any:
    """Detect the face in a *source portrait*, retrying with added margin.

    RetinaFace's anchors top out well below a face that fills the whole
    frame, so a tightly cropped headshot — a passport photo, an avatar, or
    anything from thispersondoesnotexist.com — detects at a confidence of
    zero while the same face with a little margin scores ~0.86. Webcam
    frames always have margin, so this only matters for source images, and
    padding every live frame would cost detection accuracy and time.

    Falling back to a padded copy turns "nothing happens and nobody says
    why" into a working swap. Coordinates found on the padded copy are
    translated back, so ``bbox``, ``kps`` and the landmarks always index the
    frame that was passed in — callers that only want the embedding are
    unaffected, and callers that place things on the face (hair transfer,
    masking) get usable geometry instead of numbers off the edge.
    """
    ...
    for ratio in (0.3, 0.7):
        ...
        padded = cv2.copyMakeBorder(
            frame, margin_y, margin_y, margin_x, margin_x,
            cv2.BORDER_REPLICATE,
        )
        face = get_one_face(padded)
        if face is not None:
            return _unpad_face(face, margin_x, margin_y)
    return None


def _unpad_face(face: Any, margin_x: int, margin_y: int) -> Any:
    """Shift a face detected on a padded copy back to original coordinates."""
    offset = np.array([margin_x, margin_y], dtype=np.float32)
    for attribute in ("kps", "landmark_2d_106", "landmark_3d_68"):
        points = getattr(face, attribute, None)
        if points is None:
            continue
        shifted = np.asarray(points, dtype=np.float32).copy()
        # 3D landmarks carry a depth column that must not be translated.
        shifted[:, :2] -= offset
        try:
            setattr(face, attribute, shifted)
        except Exception:
            pass

    bbox = getattr(face, "bbox", None)
    if bbox is not None:
        shifted = np.asarray(bbox, dtype=np.float32).copy()
        shifted[[0, 2]] -= margin_x
        shifted[[1, 3]] -= margin_y
        try:
            face.bbox = shifted
        except Exception:
            pass
