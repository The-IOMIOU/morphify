...
def has_ffmpeg() -> bool:
    """Whether ffmpeg is on PATH. Only video work needs it."""
...
def pre_check() -> bool:
    if sys.version_info < (3, 9):
        ...
        return False
    # ffmpeg is required to read and write video files, but live webcam mode
    # never touches it. Refusing to launch without it locked users out of the
    # main feature over a dependency it does not use, so this is a warning
    # now; the video paths check has_ffmpeg() where it actually matters.
    if not has_ffmpeg():
        print('[DLC.CORE] ffmpeg was not found on PATH - video processing is '
              'unavailable. Live camera mode works without it.')
    return True
...
def start() -> None:
    ...
    if not has_ffmpeg():
        update_status('ffmpeg is required to process video. Install it and '
