...
# Any square GFPGAN ONNX export works here: the alignment size is read from
# the model's own input shape at load time (see align_size below), so the
# file name carries no meaning beyond identifying the export. Listed
# best-first; the original hard-coded name stays first for compatibility with
# existing installs.
GFPGAN_MODEL_CANDIDATES = (
    "gfpgan-1024.onnx",
    "GFPGANv1.4.onnx",
    "gfpgan_1.4.onnx",
    "GFPGANv1.3.onnx",
...
def resolve_model_path() -> str:
    """Path of the first GFPGAN export present, or "" if none is."""
    for name in GFPGAN_MODEL_CANDIDATES:
...
def pre_check() -> bool:
    if not resolve_model_path():
        update_status(
            f"No GFPGAN ONNX model found in {models_dir}. Expected one of: "
            + ", ".join(GFPGAN_MODEL_CANDIDATES),
            NAME,
        )
        return False
    return True
        ...
        if FACE_ENHANCER is None:
            ...
            if not model_path:
                raise FileNotFoundError(
                    f"{NAME}: No GFPGAN ONNX model in {models_dir}. Expected "
                    "one of: " + ", ".join(GFPGAN_MODEL_CANDIDATES)
                )
