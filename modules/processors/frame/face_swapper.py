...
# Resolved centrally so an installed build reads models from the writable
# per-user directory instead of the read-only program folder.
from modules.paths import MODELS_DIR as models_dir  # noqa: E402

def pre_check() -> bool:
    """Verify the swap model is present. Never downloads.

    This runs from core.run() before the window exists. It used to fetch the
    554 MB FP32 model synchronously from here, which meant a first launch sat
    silently for minutes with no progress and no way to cancel — and in a
    windowed build it crashed outright, because tqdm writes to a stdout that
    PyInstaller sets to None.

    Downloading belongs to modules/model_store.py, which the UI drives behind
    a progress dialog with a disk-space check. A missing model is reported
    there instead.
    """
    try:
        ...
        logging.error(
            f"Failed to create directory {models_dir} due to permission error: {e}")
        return False
    return True
...
def get_face_swapper() -> Any:
    ...
    with THREAD_LOCK:
        if FACE_SWAPPER is None:
            ...
            fp32_path = os.path.join(models_dir, "inswapper_128.onnx")
            fp16_path = os.path.join(models_dir, "inswapper_128_fp16.onnx")
            use_fp16 = _has_ort_cuda() and os.path.exists(fp16_path)
            if use_fp16:
...
def _has_ort_cuda() -> bool:
    """Whether ONNX Runtime can actually run on CUDA.

    Kept separate from _HAS_TORCH_CUDA, which gates the torch-based paste-back
    blend and genuinely needs a CUDA torch build. Model selection and the CUDA
    graph session are pure onnxruntime concerns, so asking torch about them
    would force a ~2.5GB CUDA torch install to unlock an ONNX code path.

    get_available_providers() only reports what the build supports, so this
    also builds a throwaway session to confirm the CUDA libraries actually
    load — otherwise a missing cuDNN shows up much later as a swap failure.

    Evaluated lazily and cached: this initialises a CUDA context, which is
    not something an import should do on a CPU-only machine.
    """
    try:
        import onnxruntime as ort
        if "CUDAExecutionProvider" not in ort.get_available_providers():
            return False
        # Minimal model: one Identity node over a 1x1 float tensor.
        ...
        # onnx's defaults track the installed onnx package and can run ahead
        # of what this onnxruntime build accepts, which would fail the probe
        # for a reason that has nothing to do with CUDA. Pin both the opset
        # and the IR version to something every supported runtime understands.
...
class _SourceIdentity:
    """A stand-in for a Face. The swapper reads only ``normed_embedding``."""
...
def strengthened_source(source_face, target_face, strength: float):
    """Push the swap's identity further from the target's, for a closer likeness.

    inswapper mixes the source identity into the target's face rather than
    replacing it, which is why a swap often lands looking like neither person --
    somewhere between the two. There is no strength dial in the model, and the
    obvious one does not exist for a reason worth recording: the swapper builds
    its latent as ``normed_embedding . emap`` and then divides by that vector's
    norm, so *scaling* the embedding has no effect whatsoever. Only direction
    survives.

    So strength has to be a direction change. Extrapolating along
    ``source - target`` moves the identity away from the target's, past the
    halfway point the model would otherwise settle on. Zero leaves the model's
    own behaviour untouched; high values eventually distort, because the
    embedding leaves the region the swapper was trained on.
    """
    if strength <= 0.0 or source_face is None or target_face is None:
        return source_face
    ...
    if source_embedding is None or target_embedding is None:
        return source_face
    adjusted = source_embedding + strength * (source_embedding - target_embedding)
    norm = float(np.linalg.norm(adjusted))
    if norm < 1e-6:  # the two faces cancelled out; nothing meaningful is left
        return source_face
    return _SourceIdentity(adjusted / norm)
...
def swap_face(source_face: Face, target_face: Face, temp_frame: Frame) -> Frame:
    ...
    try:
        ...
        # Costs nothing at the default of 0.0, which leaves the model alone.
        identity_strength = float(
            getattr(modules.globals, "identity_strength", 0.0) or 0.0)
        swap_source = strengthened_source(
            source_face, target_face, max(0.0, min(1.0, identity_strength)))
        ...
        if any("DmlExecutionProvider" in p for p in modules.globals.execution_providers):
            with modules.globals.dml_lock:
                bgr_fake, M = face_swapper.get(
                    temp_frame, target_face, swap_source, paste_back=False
                )
        else:
            bgr_fake, M = face_swapper.get(
                temp_frame, target_face, swap_source, paste_back=False
